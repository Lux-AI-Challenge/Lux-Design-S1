...
type City(teamid: int, cityid: string, fuel: float, light_upkeep_init: float) =
    member _.cityid = cityid
    member _.team = teamid
    member _.fuel = fuel
    member _.CityTiles: List<CityTile> = new List<CityTile>()
    member _.light_upkeep = light_upkeep_init
...
type GameMap(width, height) =
    ...
    member self.map: Cell[,] = 
        Array2D.init width height (fun x y -> Cell(x,y)) 
...
type Unit(teamid, u_type, unitid, x, y, cooldown, wood, coal, uranium) =
    member self.Pos = Position(x, y)
    member self.team: int = teamid
    member self.id: string = unitid
